...
(def templates
  {:biff.auth/signup
   (fn [{:keys [biff.auth/link]}]
     {:subject "Sign up for bldr"
      :html (rum/render-static-markup
              [:div
               [:p "We received a request to sign up for bldr using this email address."]
   ...
   :biff.auth/signin
   (fn [{:keys [biff.auth/link]}]
     {:subject "Log in to bldr"
      :html (rum/render-static-markup
              [:div
               [:p "We received a request to log in to bldr using this email address."]
...
; :mailgun/from "bldr <contact@yourdomain.example.com>"
